...
<head>
    ...
    <link rel="icon" href="/core-docs-index/favicon.ico" type="image/x-icon">
...
<body data-clear-panel="<?= $clearClient ? '1' : '0' ?>">
    <header class="hero py-3">
        ...
        <div class="container d-flex align-items-center justify-content-between gap-2">
            <h2 class="h4 mb-0"><i class="fa-solid fa-folder-open me-2 text-primary" aria-hidden="true"></i>Projetos <span class="badge text-bg-secondary"><?= count($projects) ?></span></h2>
            <input type="search" id="search" class="form-control" placeholder="Buscar projeto ou tela" aria-label="Buscar projeto ou tela" style="max-width: 40vw;">
        </div>
    </header>
    <main class="container pb-5">

        <div class="row my-3 mx-0 p-0">
            <div class="col m-0 p-0 d-flex flex-wrap gap-3 align-items-center small text-secondary" aria-label="Legenda das telas">
            ...
            <div class="col-md-7 m-0 p-0 d-flex flex-wrap justify-content-end gap-2">
                <button type="button" id="read-paths" class="btn btn-outline-primary text-nowrap"><i class="fa-solid fa-folder-tree me-1" aria-hidden="true"></i>Read paths</button>
                <?php if ($phpMyAdminUrl): ?><a class="btn btn-outline-secondary text-nowrap" href="<?= h($phpMyAdminUrl) ?>" target="_blank" rel="noopener noreferrer"><i class="fa-solid fa-database me-1" aria-hidden="true"></i>phpMyAdmin</a><?php endif; ?>
                <button class="btn btn-outline-secondary text-nowrap" data-bs-toggle="modal" data-bs-target="#logsModal">
                    <i class="fa-solid fa-clipboard-list me-1" aria-hidden="true"></i>Logs
                    <span id="logs-count" class="badge rounded-pill text-bg-danger ms-1 d-none" aria-label="Novos registros"></span>
                </button>
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <?php foreach ($project['buttons'] as $button): ?>
                                    <a class="btn btn-sm btn-<?= $button[0] === 'Home' || $button[0] === 'Abrir portal' ? 'primary' : 'outline-primary' ?>" href="<?= h($button[1]) ?>" target="_blank" rel="noopener noreferrer"><i class="fa-solid <?= $button[0] === 'Home' ? 'fa-house' : ($button[0] === 'Temas' ? 'fa-palette' : 'fa-arrow-up-right-from-square') ?> me-1" aria-hidden="true"></i><?= h($button[0]) ?></a>
                                <?php endforeach; ?>
                                <?php if ($editorUrl): ?>
                                ...
                                <div class="d-flex flex-wrap gap-2" aria-label="<?= h($section) ?>">
                                    <?php foreach ($items as $path): ?>
                                        <a class="btn btn-sm btn-outline-<?= $isListing ? 'primary' : ($isInternal ? 'warning' : 'success') ?>"
                                            href="<?= h(screenUrl($folder, $path)) ?>" target="_blank" rel="noopener noreferrer"
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title fs-5" id="logsTitle"><i class="fa-solid fa-clipboard-list me-2 text-primary" aria-hidden="true"></i>Registro da varredura</h2><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                </div>
                <div class="modal-body">
                    <div id="log-cards" class="row g-3"></div>
                </div>
        ...
        <div class="modal fade" id="api-<?= h($project['folder']) ?>" tabindex="-1" aria-labelledby="api-title-<?= h($project['folder']) ?>" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <div>
                            <h2 class="modal-title fs-5" id="api-title-<?= h($project['folder']) ?>">Rotas API: <?= h($project['folder']) ?></h2>
                            <p class="small text-secondary mb-0">Extraídas de routes/api.php. Campos do body são exemplos ilustrativos; consulte o controller para o formato real.</p>
                        </div>
    ...
    <script id="scan-event" type="application/json">
        <?= json_encode($scanEvent, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>
    </script>
